...
class KS(Dataset):
    ...
    def __init__(
        ...
        self.nx = 120
        self.nt_total, self.nx_total = 81, 120
        ...
        self.n_simu = len(self.dataset_cache['u'])
        ...
        super(KS, self).__init__(self.root, transform, pre_transform)
    ...
    def get(self, idx):
        ...
        x_pos = np.linspace(0, 1, self.nx)
        data_traj=self.dataset_cache['u'][sim_id]
        force=self.dataset_cache['f'][sim_id]
        if self.verbose:
...
class KS1DSimple(KS):
    ...
    '''
    def __init__(self, *args, device='cpu', rescaler=None, 
                RESCALER_u=10,
                RESCALER_f=0.1,
                stack_u_and_f=False, pad_for_2d_conv=False, **kwargs): 
        ...
        if rescaler is None:    
            ...
            self.RESCLAER_u = RESCALER_u
            self.RESCALER_f = RESCALER_f
    ...
    def get(self, idx, use_normalized=True):
        ...
        sim_id, time_id = divmod(idx, self.time_stamps_effective)
        x_pos = np.linspace(0, 1, self.nx)
        data_traj=self.dataset_cache['u'][sim_id]/self.RESCLAER_u
        force=self.dataset_cache['f'][sim_id]/self.RESCALER_f
