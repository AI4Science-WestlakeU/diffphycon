...
class Unet2D(nn.Module):
    ...
    def __init__(
        ...
        attn_dim_head = 32,
        attn_heads = 4, 
        condition_on_residual = None,  
    ):
        super().__init__()
        # customed
        self.condition_on_residual = condition_on_residual
        if self.condition_on_residual:
            if self.condition_on_residual == 'residual_gradient':
                assert out_dim == 2 # u, f, partial_u L, partial_f L
            else:
    ...
    def forward(self, x, time, x_self_cond = None, residual=None):
