...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.model = model
        if eval_2ddpm:
            self.model_joint, self.model_thetas = model
            self.channels = self.model_joint.channels
            self.self_condition = self.model_joint.self_condition
        else:
            self.model = model
            self.channels = self.model.channels
            self.self_condition = self.model.self_condition
        self.is_w_model = self.channels == 2
        self.image_size = image_size
        self.frames = frames
        self.objective = objective
        self.standard_fixed_ratio = standard_fixed_ratio
        self.coeff_ratio = coeff_ratio
        self.eval_2ddpm = eval_2ddpm
    ...
    def model_predictions(self, shape, x, t, x_self_cond = None, clip_x_start = False, rederive_pred_noise = False, design_fn = None, design_guidance = "standard", low=None, init=None, init_u=None):
        model_output_joint = self.model_joint(x, t, x_self_cond)
        x_w = x[:, :, 3:5]
        model_outout_w = self.model_thetas(x_w, t, x_self_cond)
        pred_noise_w = torch.zeros_like(model_output_joint, device = x.device) # [b, 32, 6, h, w]
        pred_noise_w[:,:, 3:5] = model_outout_w
        maybe_clip = partial(torch.clamp, min = -1., max = 1.) if clip_x_start else identity

        if self.objective == 'pred_noise': 
            pred_noise = model_output_joint
            x_start = self.predict_start_from_noise(x, t, model_output_joint)
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, design_fn=None, design_guidance="standard", return_all_timesteps=None, init=None, init_u=None, control=None, low=None, device=None):
        b, f, c, h, w = shape 
    ...
    @torch.no_grad()
    def ddim_sample(self, shape, design_fn=None, design_guidance="standard", init=None, init_u=None, control=None, low=None, device=None):
    ...
    @torch.no_grad()
    def sample(self, batch_size = 16, design_fn = None, design_guidance="standard", init=None, init_u=None, control=None, low=None, device = None):
        ...
        return sample_fn(sample_size, design_fn, design_guidance, init=init, init_u=init_u, control=control, low=low, device = device)
    ...
    def p_losses(self, state_start, t, noise = None):
        ...
        if self.objective == 'pred_noise':
            ...
            # print("loss_state + loss_thetas: ", loss_state, loss_thetas, loss)
    ...
    def __init__(
        ...
        *,
        ...
        if is_schedule == True:
            ...
            # self.scheduler = lr_scheduler.MultiStepLR(self.opt, milestones=[self.resume_step + 2, self.resume_step + 5,  self.resume_step + 8], gamma=0.1)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                if self.step != 0 and self.step % 10 == 0:
                    ...
                    # p.print(f'loss: {total_loss:.4f}, LR: {self.opt.param_groups[0]["lr"]}', tabs=0, is_datetime=None, banner_size=0, end=None, avg_window=1, precision="millisecond", is_silent=False)
