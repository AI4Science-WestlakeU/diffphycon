...
# Agent
agent = SAC(int(s/2)+int(s/2)+1, int(s/2), args)

pde_1d_surrogate_model_checkpoint="/checkpoints/pde_1d/partial_ob_partial_ctr_1-step"
milestone=500
...
for i_episode in tqdm(range(args.num_episode)):
    ...
    if (i_episode + 1) % 25 == 0 and args.eval == True:
        ...
        for i in range(num_t):
            ...
            next_state_surrogate = simu_surrogate_model.simulation(ut=torch.tensor(state_surrogate[:,1:int(s/2)+1], device=device).float(), ft=torch.tensor(action_surrogate, device=device).float()) \
                                    .cpu().numpy().reshape(batch_size, int(s/2))
        ...
        torch.save({'logs':logs, 'policy_state_dict': policy_para, 'args':args}, 'cp/SAC_policy_track_pob_pctr{}_{}_{}'.format(date, args.seed, i_episode))
        with open('cp/SAC_policy_track_pob_pctr{}_{}.txt'.format(date, args.seed), 'w') as f:
            for item in logs_txt:
                f.write("%s\n" % item)
        print('Checkpoint: cp/SAC_policy_track_pob_pctr{}_{}_{}'.format(date, args.seed, i_episode))
