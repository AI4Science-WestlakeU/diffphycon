...
# Agent
agent = SAC(int(s/2)+int(s/2)+1, s, args)

pde_1d_surrogate_model_checkpoint="checkpoints/pde_1d/partial_ob_full_ctr_1-step"
milestone=500
simu_surrogate_model=Simu_surrogate_model(path=pde_1d_surrogate_model_checkpoint,device=device,s_ob=64,milestone=milestone)
...
dataset = h5py.File("/data/data_1d_burgers/free_u_f_1e5/burgers_train.h5", 'r')['train']
...
for i in range(num_t):
    ...
    action = f_data[:, i]  
    if True:  
        uT_reward = (-1) * torch.sum((u_target-u_data[:, i+1])**2, -1) / s * 2
    else:  
        uT_reward = 0  
...
# Test data
test_dataset = h5py.File("/data/data_1d_burgers/free_u_f_1e5/burgers_test.h5", 'r')['test']
...
test_u_data_full = test_u_data
test_u_data = torch.cat((test_u_data[:, :, :int(s/4)], test_u_data[:, :, int(3*s/4):]), -1)
test_dl = DataLoader(TensorDataset(torch.cat((test_u_data_full, test_f_data), 1)), batch_size = args.test_batch_size, shuffle = True)
...
for i_episode in tqdm(range(args.num_episode)):
    ...
    if args.online:
        ...
        u_data_full = np.array(data[:, :num_t])
        u_full_t = u_data_full[:, 0]
        u_data = np.concatenate((u_data_full[:, :, :int(s/4)], u_data_full[:, :, int(3*s/4):]), -1)
        state = np.concatenate((np.zeros_like(u_data[:,0,[0]]), u_data[:,0], u_data[:,-1]), -1).reshape(-1,s+1)
        batch_size = data.shape[0]
        for i in range(num_t):
            action = agent.select_action(state).reshape(batch_size, 1, s)
            if args.surrogate: 
                next_state = simu_surrogate_model.simulation(ut=torch.tensor(state[:,None,1:int(s/2)+1], device=device).float(), ft=torch.tensor(action, device=device).float()) \
                                        .cpu().numpy().reshape(batch_size, int(s/2))
            else:
                u_full_t = burgers_numeric_solve(torch.tensor(u_full_t, device=device), torch.tensor(action, device=device).float(), visc=0.01, T=delta_t, dt=1e-4, num_t=1)\
                                [torch.arange(batch_size), torch.arange(batch_size), -1].cpu().numpy().reshape(batch_size, s)
                next_state = np.concatenate((u_full_t[:, :int(s/4)], u_full_t[:, int(3*s/4):]), -1)
            if True:
                uT_reward = (-1) * np.sum((next_state-u_data[:,-1])**2, -1) / s * 2
            else:
                uT_reward = 0
            ...
            memory.push_batch(list(zip(state.reshape(batch_size, -1), action.reshape(batch_size, -1), reward.reshape(batch_size),\
    ...
    if (i_episode + 1) % 25 == 0 and args.eval == True:
        ...
        for i in range(num_t):
            # solver:
            action = agent.select_action(state, eval=True).reshape(batch_size, 1, s)
            u_full_t = burgers_numeric_solve(torch.tensor(u_full_t, device=device), torch.tensor(action, device=device).float(), visc=0.01, T=delta_t, dt=1e-4, num_t=1)\
                            [torch.arange(batch_size), torch.arange(batch_size), -1].cpu().numpy().reshape(batch_size, s)     
            next_state = np.concatenate((u_full_t[:, :int(s/4)], u_full_t[:, int(3*s/4):]), -1)
            if i == num_t-1:
                uT_reward = (-1) * np.sum((next_state-u_data[:,-1])**2) / batch_size / s * 2
            else:
                uT_reward = 0
            ...
            # surrogate model:
            action_surrogate = agent.select_action(state_surrogate, eval=True).reshape(batch_size, 1, s)
            next_state_surrogate = simu_surrogate_model.simulation(ut=torch.tensor(state_surrogate[:,None,1:int(s/2)+1], device=device).float(), ft=torch.tensor(action_surrogate, device=device).float()) \
                                    .cpu().numpy().reshape(batch_size, int(s/2))
            #ut [batch_size,1,ns], ft [batch_size,1,128], u_{t+1} [batch_size,1,ns]
            next_state_true = burgers_numeric_solve(torch.tensor(state_true, device=device), torch.tensor(action_surrogate, device=device).float(), visc=0.01, T=delta_t, dt=1e-4, num_t=1)\
                            [torch.arange(batch_size), torch.arange(batch_size), -1].cpu().numpy().reshape(batch_size, s)   
            next_state_true_quarter = np.concatenate((next_state_true[:, :int(s/4)], next_state_true[:, -int(s/4):]), -1)
            next_state_surrogat_all.append(next_state_surrogate)
            next_state_true_all.append(next_state_true_quarter)
            if i == num_t-1:
                uT_reward = (-1) * np.sum((next_state_true_quarter-u_data[:,-1])**2) / batch_size / s * 2
            else:
                uT_reward = 0
        ...
        torch.save({'logs':logs, 'policy_state_dict': policy_para, 'args':args}, 'cp/SAC_policy_track_pob_fctr{}_{}_{}'.format(date, args.seed, i_episode))
        with open('cp/SAC_policy_track_pob_fctr{}_{}.txt'.format(date, args.seed), 'w') as f:
            for item in logs_txt:
                f.write("%s\n" % item)
        print('Checkpoint: cp/SAC_policy_track_pob_fctr{}_{}_{}'.format(date, args.seed, i_episode))
