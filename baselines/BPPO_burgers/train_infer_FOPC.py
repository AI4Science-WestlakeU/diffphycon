...
if __name__ == "__main__":
    ...
    parser.add_argument('--pde_1d_surrogate_model_checkpoint', type=str, default='surrogate_model/FOPC/', metavar='N',
                        help='test batch size (default: 256)')
    ...
    def evaluate(seed, eval_episodes, s_0, test_target, test_u_full):
        ...
        for j in range(eval_episodes):
            ...
            if j==0:
                ...
                state_full = (s_feedback[:,0]/RESCALER).to(device)
                
            target = state_current[:,-128:]*RESCALER
            
            a = select_action(state_current, is_sample=False)
            
            a[:,int(s/4):int(3*s/4)] = 0
            
            in_surr_model = state_full
            
            f_sum[:,j] = a*RESCALER
            
            if j==0:
                u_sum[:,0] = in_surr_model*RESCALER
            
            s_feedback = simu_surrogate_model.simulation(ut=in_surr_model*RESCALER,ft=a.unsqueeze(1)*RESCALER)
            
            s_feedback_solver = burgers_numeric_solve_free(in_surr_model*RESCALER, a.unsqueeze(1)*RESCALER, visc=0.01, T=0.1, dt=1e-4, num_t=1)
            
            output_solver = s_feedback[:,0]
            
            u_sum[:,j+1] = s_feedback_solver[:,0]
            
            r = (s_feedback_solver[:,0]-target).square().mean(-1)
            rl2 = rel_error(s_feedback_solver[:,0], target)
    ...
    simu_surrogate_model = Simu_surrogate_model(path=args.pde_1d_surrogate_model_checkpoint,device=device,s_ob=128)
    
    test_dataset = h5py.File("../../1D_data/free_u_f_1e5_front_rear_quarter/burgers_test.h5", 'r')['test']
    test_u_data, test_f_data = ((torch.tensor(np.array(test_dataset['pde_11-128'])).float())/RESCALER)[:1], ((torch.tensor(np.array(test_dataset['pde_11-128_f'])).float())/RESCALER)[:1]
    
    test_u_full = test_u_data
    test_target = test_u_data[:,-1]
    
    path = '/folder/baseline/BPPO/u_f_BC/u_d/'
