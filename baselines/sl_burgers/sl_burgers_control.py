...
if __name__=='__main__':
    ...
    #load_data
    test_dataset = h5py.File("data/free_u_f_1e5_front_rear_quarter/burgers_test.h5", 'r')['test']
    ...
    pde_1d_surrogate_model_checkpoint="checkpoints/full_ob_partial_ctr_1-step"
    milestone=500
    model=Simu_surrogate_model(path=pde_1d_surrogate_model_checkpoint,device=device,s_ob=128,milestone=milestone)
    ...
    for i in range(50):
        ...
        f_nn, t_nn=burgers_nn_control_2(u_in.cuda(), u_fin.cuda(), s, args.reward_f, model_f=load_model_f, model_u=load_model_u, model_trans=load_model_trans, epochs=epochs, lr=lr, Nt=num_t, dt=delta_t, dx=delta_x, lamb1=args.lamb1, lamb2=args.lamb2, gtol=gtol, ftol=ftol)
        f_nn[:, int(s/4):int(3*s/4)] = 0
        logs['f_nn'].append(f_nn)
        logs['t_nn'].append(t_nn)
        with HiddenPrints():
            u_nn=burgers_numeric_solve(u_in.unsqueeze(0).cuda(), f_nn.unsqueeze(0), visc=0.01, T=1.0, dt=1e-4, num_t=num_t)
        u_nn = u_nn.squeeze()
        u_nn_surrogate=burgers_nn_solve(u_in.cuda(), f_nn.unsqueeze(0), load_model_f, load_model_u, load_model_trans, s, num_t)
        logs['u_nn'].append(u_nn)
        u_nn_surrogate_all.append(u_nn_surrogate.detach().cpu().numpy())
        u_nn_all.append(u_nn[1:].cpu().numpy())
        logs['u_nn'].append(u_nn)
        er_u_nn, er_f_nn, er_uf_nn = objective_value(u_nn[-1].cpu().numpy(), f_nn.cpu().numpy(), u_fin.numpy(), s, args.reward_f)
