...
try:
    from data_ks_1d import KS1DSimple, KS
except:
    from model.pde_1d_surrogate_model.data_ks_1d import KS1DSimple, KS
import argparse
import pdb
...
if __name__=='__main__':
    ...
    device='cuda',
    rescaler=RESCALER,
    RESCALER_u=RESCALER_u,
    RESCALER_f=RESCALER_f
    )
    test_dataset= KS1DSimple(
    dataset="ks",
    input_steps=1,
    output_steps=10,
    time_interval=1,
    ...
    for epoch in tqdm.trange(1, epochs+1):
        ...
        if epoch%1==0:
            print('!!!!!!!!!!!!!!!!!! model is saved at:',args.exp_path+f"model_f-{epoch}.pt")
