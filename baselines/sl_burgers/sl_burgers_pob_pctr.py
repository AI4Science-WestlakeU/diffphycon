...
if __name__=='__main__':
    ...
    pde_1d_surrogate_model_checkpoint="checkpoints/partial_ob_partial_ctr_1-step"
    milestone=500
    ...
    for i in range(50):
        ...
        f_nn_, t_nn=burgers_nn_control_2(u_in.cuda(), u_fin.cuda(), int(s/2), args.reward_f, model_f=load_model_f, model_u=load_model_u, model_trans=load_model_trans, epochs=epochs, lr=lr, Nt=num_t, dt=delta_t, dx=delta_x, lamb1=args.lamb1, lamb2=args.lamb2, gtol=gtol, ftol=ftol)
        f_nn = torch.zeros((10, s)).cuda()
        f_nn[:, :int(s/4)] = f_nn_[:, :int(s/4)]
        f_nn[:, -int(s/4):] = f_nn_[:, -int(s/4):]
    ...
    torch.save({'logs': logs,
                'args': args
                }, 'logs/test_pob_pctr_{}'.format(date))
